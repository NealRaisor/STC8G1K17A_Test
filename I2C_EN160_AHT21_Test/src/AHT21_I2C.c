...
const u8 code AHT21_MEAS_CMD[3] = {0xAC, 0x33, 0x00}; // Measure CMD

// 仅本文件使用
static u8 xdata AHT21_Data_Buf[AHT21_DATA_LEN]; // AHT21 data buf
static u32 Tmp         = 0;                     // temp save var
static bit Flag_UnRead = 0;                     // AHT21 read data flag bit 0: Read over 1: Didn't read

// 异步延时，定时器0中减少，本文件中设置值以及读取值
u8 AHT21_Delay = 0; // dalay time var, Added in this document and reduced in timer interrupt function.
...
void AHT21_Init(void)
{
    u8 status = 0;
    // 清空数据数组
    for (status = 0; status < AHT21_DATA_LEN; ++status) {
        AHT21_Data_Buf[status] = 0;
    }
    // 发送重启命令
    status = AHT21_SOFT_RESET_CMD;
    I2C_WriteNBytes(AHT21_ADDR, I2C_NO_MEM_ADDR, &status, 1);
    delay_ms(20); // 阻塞延时20ms
    // 判断校准位
    status = AHT21_Get_Status();
...
void AHT21_Measure(void)
{
    ...
    I2C_WriteNBytes(AHT21_ADDR, I2C_NO_MEM_ADDR, AHT21_MEAS_CMD, sizeof(AHT21_MEAS_CMD));
    AHT21_Delay = 80; // 延时
    Flag_UnRead = 1;  // 设置标志位
}
